...
function resolveElectronPreloadPath() {
    const candidates = [
...
function serializeSafeStorageJson(data) {
...
function createCorruptBackupPath(storagePath) {
...
async function readDisplaySettings() {
    ...
    try {
        ...
        return sanitizeDisplaySettings(JSON.parse(content));
    }
    catch (error) {
...
async function writeDisplaySettings(settings) {
    await ensureStorageDirectory();
    const filePath = path.join(getStorageDirectory(), DISPLAY_SETTINGS_FILE);
...
function sanitizeDisplaySettings(value) {
...
function getCenteredWindowedBounds(width, height, win) {
...
function getFullDisplayBounds(win) {
    const currentDisplay = getDisplayForWindow(win);
    return { ...currentDisplay.bounds };
}
function getInitialWindowBounds(settings) {
...
function enterRealFullscreen(win) {
...
function enterWindowed(win, settings) {
    const { width, height } = RESOLUTION_SIZES[settings.resolution];
    win.setResizable(true);
    win.setFullScreen(false);
    if (win.isMaximized()) {
...
function applyDisplaySettings(win, settings) {
...
ipcMain.handle('storage:readJson', async (event, fileName) => {
    assertTrustedIpcSender(event);
    await ensureStorageDirectory();
...
ipcMain.handle('storage:writeJson', async (event, fileName, data) => {
    assertTrustedIpcSender(event);
    await ensureStorageDirectory();
    const storagePath = getStoragePath(fileName);
...
ipcMain.handle('display:setSettings', async (event, requestedSettings) => {
    assertTrustedIpcSender(event);
